...
function ResultsContent({ id }: { id: string }) {
    ...
    if (loading) return <div style={{ display: "flex", justifyContent: "center", padding: "120px 24px" }}><div className="spinner" /></div>;
    if (!quiz) return null;

    const total = quiz.questions.length;
    const pct = Math.round((score / total) * 100);

    const getResultColor = () => pct >= 80 ? "#10d98a" : pct >= 50 ? "#ffb84d" : "#ff4d6d";
    ...
    return (
        <div style={{ position: "relative", minHeight: "calc(100vh - 72px)" }}>
            ...
            <div style={{ position: "relative", zIndex: 1, maxWidth: "800px", margin: "0 auto", padding: "50px 24px 80px" }}>
                <div className="glass-strong animate-slide-up" style={{ padding: "48px", textAlign: "center", marginBottom: "32px" }}>
                    ...
                    <div style={{ display: "inline-flex", alignItems: "baseline", gap: "4px", padding: "24px 48px", borderRadius: "20px", background: `rgba(${pct >= 80 ? "16,217,138" : pct >= 50 ? "255,184,77" : "255,77,109"}, 0.08)`, border: `1px solid ${getResultColor()}30`, marginBottom: "24px" }}>
                ...
                <h2 style={{ fontSize: "20px", fontWeight: 700, marginBottom: "20px" }}>📋 Question Breakdown</h2>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                    {quiz.questions.map((q, i) => {
                        ...
                        return (
                            <div key={q.id} className="glass animate-fade-in" style={{ padding: "24px", borderColor: isCorrect ? "rgba(16,217,138,0.2)" : "rgba(255,77,109,0.2)" }}>
                                <div style={{ display: "flex", gap: "12px", alignItems: "flex-start", marginBottom: "16px" }}>
                                    <div style={{ width: "28px", height: "28px", borderRadius: "50%", background: isCorrect ? "rgba(16,217,138,0.2)" : "rgba(255,77,109,0.15)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", flexShrink: 0, marginTop: "2px" }}>
                                        {isCorrect ? "✓" : "✗"}
                                    </div>
                                    <div>
                                        <div style={{ fontSize: "12px", fontWeight: 600, color: "rgba(175,175,210,0.5)", marginBottom: "4px" }}>Q{i + 1}</div>
                                        <p style={{ fontSize: "15px", fontWeight: 600, color: "#f0f0ff", lineHeight: 1.5 }}>{q.questionText}</p>
                                    </div>
                                </div>
                                ...
                                <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "16px" }}>
                                    {OPTIONS.map((opt) => {
                                        ...
                                        if (isCorrectOpt) { bg = "rgba(16,217,138,0.08)"; borderColor = "rgba(16,217,138,0.4)"; textColor = "#10d98a"; }
                                        else if (isUserOpt && !isCorrect) { bg = "rgba(255,77,109,0.07)"; borderColor = "rgba(255,77,109,0.3)"; textColor = "#ff4d6d"; }

                                        return (
                                            <div key={opt} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 14px", borderRadius: "8px", border: `1px solid ${borderColor}`, background: bg, fontSize: "14px", color: textColor }}>
